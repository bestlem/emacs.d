...
Another issue is that xonsh does not understand escaped spaces which shell-command does it needs the windows treatment. (and as not using a GPL shgell or OS Emacs won't fix that)
*** Aquamacs
:PROPERTIES:
...
:END:
As noted the aquamacs way might be better
...
:END:
*** Get a icon at the front of a string
:PROPERTIES:
:ID:       org_mark_mini12.local:20201223T175853.557567
:END:
Add a treemacs icon to the front - needs to be after file management
#+NAME: org_mark_mini12.local_20201223T175853.523006
#+begin_src emacs-lisp
(defun mwb-icon-string (icon-file-extension text )
  ;; Get an icon for the icon-file-extension e.g. "el" for an emacs icon and
  ;; add to front of string text
  (concat
