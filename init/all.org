...
The order here does not matter much. On second thoughts especially with desktop it does. We need the environment set up befoire we load files. Apperance and keys are later. Moset things are on :defer t so only start when needed. :defer with time is also problematoic as it makes :after fail; which is why some :config have use-package in them.
