...
It turns out Emacs supports multiple themes being active at the same time, which I’m sure is convenient sometimes but becomes a right nuisance when attempting to switch themes IMO. Add a utility function to disable all currently enabled themes first. gfdgdfg
...
(pretty-hydra-define hydra-jp-window (:foreign-keys warn :title jp-window--title :quit-key "q")
  ("Actions"
   (("TAB" other-window "switch")
    ("ax" ace-delete-window "delete")
    ("am" ace-delete-other-windows "maximize")
    ("as" ace-swap-window "swap")
    ("aa" ace-select-window "select"))
   ...
   "New Frames"
   (("m" (mwb-new-frame ("*Messages*"))  :exit t))

   "Other"
   (("0" jp-zoom-default "reset zoom")
    ("t" quick-switch-themes* "cycle themes")
    ("p" package-quickstart-refresh "refresh quickstart packages" :exit t)
...
:END:
This is hardcoded. See all.org anmd desktop for other ways. Aquamacs just restores frames using revive.
