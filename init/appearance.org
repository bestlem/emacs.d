...
Note this shows up in org-mode compare code to text - so have to make fixed-font a proportion of variable-font even though no direct comparison
...
(let ((font-found (font-candidate
  ...
  (set-face-attribute 'default nil
                      :height 130
                      :family font-found)
  ;; Monospaced typeface - not default
...
(when (and window-system (not *aquamacs*))
  ...
  (set-frame-position (selected-frame) 50 132))
#+end_src
