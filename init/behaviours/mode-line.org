...
#+end_src
**** Deal with active window or not
:PROPERTIES:
:ID:       org_mark_mini20.local:20210606T221600.822244
:END:
We need to switch the background face for active mode line. (jp- is easier as each segment does this
 as does some doom segments)
 Issue looks like some faces don't set the background
* Spaceline -no tangle
:PROPERTIES:
