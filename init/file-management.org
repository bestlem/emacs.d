...
:END:
 ...
 Issue is that recentf-exclude is used by passing the expanded filename(which makes senss)  and the file list uses the shortened name which also makes sense at least in the file as then is independent of ~
 ...
 The problem is that we have regexq
