...
There is a general idea as in the SO answer quoted above as to put your keys into a minor mode map that you can turn and off. Aquamacs comment for osx-map-mode suggests the same. But there is a mix here as I want to be able to override my keyz``s. As there is no real use of Alt key in Emacs it is an independent map (except for the iso-transl mess)/
...
** Global hydra
:PROPERTIES:
:ID:       org_2020-12-05+00-00:A17FFF67-3F29-454D-91D2-DA54007D07E2
:END:
Odd things needed
...
(use-package emacs
  ...
  :pretty-hydra
  (hydra-global-tools
   (:foreign-keys warn :title (with-octicon "tools" "Global tools" 1 -0.05) :color teal :quit-key "q")
   ("Hydrae"
  ...
  :bind ("H-q" . hydra-global-tools/body))
#+end_src
