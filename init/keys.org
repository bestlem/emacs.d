...
:END:
keys.org has to be before other things so the delete key can be overridden - ie set to macports or not in one place. (Possibly not they all are based on [deletechar] - that change was in 2001 so my original mapping was begore then, but Aquamacs does this as well so leave.)
...
Using a minor mode map means it needs to be loaded before other minors that alter the keys - e.g. org-src-mode . Which means that keys.org has to be non noweb, which is no problem.
...
:END:
Do the main bindings
#+NAME: org_mark_mini20.local_20220701T094925.469704
#+begin_src emacs-lisp
(use-feature mac-key-mode
  :unless *aquamacs*
  :straight (:host github :repo "bestlem/mac-key-mode") ; not a fork as this is old
  :demand
  :diminish
...
#+RESULTS[78fc5fc245af10de56c6742bbb4a9ff0ff8e658e]: org_mark_mini20.local_20210811T144548.688365
#+begin_EXAMPLE
hydra-search/body
#+end_EXAMPLE
