...
(use-package-elpa magit
  ...
  (pretty-hydra-define+ jp-git
    ...
    ("Magit"
     (("s" mwb-magit-status "magit status")
...
:END:
[[https://github.com/dgutov/diff-hl][diff-hl]] Does not need magit
...
Also gets used by spaceline-all-the-icons but not in my elpa so not used.
